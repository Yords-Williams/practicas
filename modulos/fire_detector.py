...
class FireDetectionSystem:
    """Detector de incendios para video en tiempo real basado en color y textura."""
    ...
    def detect_fire(self, frame) -> Tuple[bool, float, str]:
        ...
        if area >= self.config.get("min_area", 180):
            details = f"Posible incendio detectado con área {int(area)} px"
    ...
    def send_report(self, incident: Dict):
        ...
        for recipient in recipients:
            ...
            if recipient.get("phone"):
                print(f"[FIRE] SMS/WhatsApp simulado para {recipient['phone']}: {message}")
